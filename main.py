...
async def test_act():
    talk: StartPlayTTS = StartPlayTTS(text="I feel good today, thank you")
    express: PlayExpression = PlayExpression(express_name="codemao9")
    hug: PlayAction = PlayAction(action_name='012')
    h1 = talk.execute()
    h2 = express.execute()
    h3 = hug.execute()
    await h1
...
async def test_play_action():
    ...
    block: PlayAction = PlayAction(action_name='action_004')
